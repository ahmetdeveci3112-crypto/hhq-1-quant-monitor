...
export const ActiveSignalsPanel: React.FC<ActiveSignalsPanelProps> = ({ signals, pendingEntries = [], opportunities = [], onMarketOrder, entryTightness = 1.0, minConfidenceScore = 40, priceFlashMap = {}, signalCounts }) => {
    ...
    // Faz 2 UI-Redesign: Track which signal cards show detail collapse
    ...
    // Faz 1 UI-Redesign Fix: Panel header counters derive from RENDERED arrays
    // activeSignals is post-qualityFilter, confirmedPending/unconfirmedPending are full pending lists
    // Header total = visible items in this panel. Long/Short = visible items by side.
    const visibleSignalCount = activeSignals.length + confirmedPending.length + unconfirmedPending.length;
    const actionableSignalCount = visibleSignalCount;
    const actionableLongCount = activeSignals.filter(s => s.signalAction === 'LONG').length
      + confirmedPending.filter((e: any) => e.signalAction === 'LONG').length
      + unconfirmedPending.filter((e: any) => e.signalAction === 'LONG').length;
    const actionableShortCount = activeSignals.filter(s => s.signalAction === 'SHORT').length
      + confirmedPending.filter((e: any) => e.signalAction === 'SHORT').length
      + unconfirmedPending.filter((e: any) => e.signalAction === 'SHORT').length;
    ...
    const SignalCard = ({ signal, key: _key }: { signal: CoinOpportunity; key?: string }) => {
        ...
        return (
            <div className={`rounded-lg border transition-colors ${isLong ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-rose-500/5 border-rose-500/30'
                }`}>
                {/* ═══ Faz 2: Seviye 1 — Her zaman görünür ═══ */}
                <div className="p-3">
                    ...
                    <button
                        onClick={() => setExpandedSignals(prev => {
                            ...
                            next.has(signal.symbol) ? next.delete(signal.symbol) : next.add(signal.symbol);
                            return next;
                        })}
                        className="w-full flex items-center justify-center gap-1 text-[10px] text-slate-500 hover:text-slate-300 py-1 mt-1 transition-colors"
                    >
                        {expandedSignals.has(signal.symbol) ? (
                            <><ChevronUp className="w-3 h-3" />Detayı Gizle</>
                        ) : (
                            <><ChevronDown className="w-3 h-3" />Kalite & Karar Detayı</>
                        )}
                    </button>
                </div>

                {/* ═══ Faz 2: Seviye 2 — Kalite + Karar Detayı ═══ */}
                {expandedSignals.has(signal.symbol) && (
                    <div className="border-t border-slate-800/30 px-3 pb-3 space-y-2">
                        {/* Kalite Badges */}
                        <div className="pt-2">
                            <div className="text-[9px] uppercase tracking-wider text-slate-500 mb-1">Kalite</div>
                            <div title={qualityTooltip}>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-[10px] mt-1.5">
                                ...
                                <span className="text-cyan-400" title={trailTitle}>
                        ...
                        <div className="pt-2 border-t border-slate-800/20">
                            <div className="text-[9px] uppercase tracking-wider text-slate-500 mb-1">Karar Detayı</div>
        ...
        <div className="bg-[#0d1117] border border-slate-800/50 rounded-lg overflow-hidden">
            ...
            <div className="flex items-center gap-3 border-b border-slate-800/30 bg-slate-950/40 px-4 py-2.5 overflow-x-auto">
                ...
                <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-[10px] text-slate-500">Sırada</span>
                </div>
                <div className="h-4 w-px bg-slate-800"></div>
                <div className="flex items-center gap-2 text-xs">
                    <span className="text-amber-300 font-semibold">{unconfirmedPending.length}</span>
                    <span className="text-[10px] text-slate-500">Kontrol Altında</span>
                </div>
                <div className="h-4 w-px bg-slate-800"></div>
                {/* Faz 1 UI-Redesign: Sayaç→Render Array Mapping
                    Hazır = activeSignals.length
                    Sırada = confirmedPending.length
                    Kontrol = unconfirmedPending.length
                    Toplam = üçünün doğrudan toplamı (render array sum)
                */}
                <div className="flex items-center gap-2 text-xs">
                    <span className="text-white font-semibold">{activeSignals.length + confirmedPending.length + unconfirmedPending.length}</span>
                ...
                <div className="border-b border-slate-800/30 bg-slate-950/30 px-4 py-3">
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <div className="text-xs font-semibold text-white">Sırada</div>
                                <div className="text-[10px] text-slate-400">Onaylanmış, giriş koşulu bekleyen sinyaller</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 text-xs">
                ...
                <div className="border-b border-slate-800/30 bg-slate-950/20 px-4 py-3">
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <div className="text-xs font-semibold text-white">Kontrol Altında</div>
                                <div className="text-[10px] text-slate-400">Henüz onaylanmamış, izlenen giriş emirleri</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 text-xs">
                            <span className="text-amber-300 font-semibold">{unconfirmedPending.length}</span>
            ...
            {/* Faz 1 UI-Redesign: Global empty state — tüm kuyruk boşsa tek mesaj */}
            {activeSignals.length === 0 && confirmedPending.length === 0 && unconfirmedPending.length === 0 && (
                <div className="px-4 py-12 text-center">
                    <Zap className="w-10 h-10 mx-auto mb-3 text-slate-700" />
                    <p className="text-sm text-slate-500 font-medium">Giriş kuyruğu boş</p>
                    <p className="mt-1 text-[11px] text-slate-600">Piyasa taranıyor — uygun sinyal üretildiğinde burada görünecek.</p>
                </div>
            )}

            {/* Faz 1 UI-Redesign: Hazır section — boşsa collapse, doluysa normal render */}
            {(activeSignals.length > 0 || confirmedPending.length > 0 || unconfirmedPending.length > 0) && (
                <>
                    {activeSignals.length === 0 ? (
                        /* Hazır section collapsed — tek satır bilgilendirme */
                        <div className="px-4 py-2.5 border-b border-slate-800/30 bg-slate-950/20">
                            <div className="flex items-center gap-2 text-[11px] text-slate-500">
                                <Zap className="w-3.5 h-3.5 opacity-40" />
                                <span>Şu an hemen giriş yapılacak sinyal yok.</span>
                                {pendingSignals.length > 0 && (
                                    <span className="text-slate-400">Bekleyen girişler koşullar oluştuğunda buraya taşınacak.</span>
                                )}
                            </div>
                        </div>
                    ) : (
                        <>
                            <div className="px-4 py-3 border-b border-slate-800/30 bg-slate-950/20">
                                <div className="flex items-center justify-between gap-3">
                                    <div>
                                        <div className="text-xs font-semibold text-white">Hazır Sinyaller</div>
                                        <div className="text-[10px] text-slate-400">Hemen giriş yapılabilecek sinyaller</div>
                                    </div>
